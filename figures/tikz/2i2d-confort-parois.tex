...
\end{scope}
\node[txt, anchor=north] at (1.60,-0.62) {$T_{\text{parois}}$ ($^\circ$C)};
\node[txt, anchor=south west, inner sep=1pt] at (-0.55,3.50) {$T_{\text{air}}$ ($^\circ$C)};

%% ================= légende, à droite du diagramme ===========================
\def\xc{3.80}
\fill[solideE!18] (\xc,3.28) rectangle (\xc+0.34,3.52);
\draw[solideE, line width=1.2pt] (\xc,3.40) -- (\xc+0.34,3.40);
\node[txt, anchor=north west, align=left] at (\xc+0.42,3.56)
  {zone de confort\\ ressenti $\approx 18\ ^\circ$C};
\node[txt, anchor=north west, align=left] at (\xc,2.55)
  {\textbf{A} — avant isolation\\ parois $15\ ^\circ$C, air $21\ ^\circ$C};
\node[txt, anchor=north west, align=left] at (\xc,1.60)
  {\textbf{B} — après isolation\\ parois $19\ ^\circ$C, air $17\ ^\circ$C};
\node[txt, anchor=north west, align=left, text=solideD] at (\xc,0.65)
  {même sensation, mais\\ $4\ ^\circ$C de consigne en moins};

%% ================= relation encadrée ========================================
\node[draw=black!70, line width=0.7pt, rounded corners=2pt, fill=black!3, align=center,
      inner sep=4pt, anchor=north] at (1.75,-0.98)
