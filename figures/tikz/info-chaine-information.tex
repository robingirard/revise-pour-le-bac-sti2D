...
\begin{tikzpicture}[x=1cm,y=1cm,
  bloc/.style={draw=solideB, line width=1.2pt, rounded corners=4pt, fill=white,
               minimum width=2cm, minimum height=2.15cm, inner sep=3pt,
  ...
  bord/.style={font=\scriptsize, text width=1.35cm, inner sep=1pt,
               execute at begin node={\hyphenpenalty=10000\exhyphenpenalty=10000}}]

%% ---- les trois blocs (même gabarit imposé) ---------------------------
\node[bloc] (acq) at (0,0)    {\textbf{ACQUÉRIR}\\[3pt] capteurs\\ boutons, clavier};
\node[bloc] (tra) at (2.35,0) {\textbf{TRAITER}\\[3pt] carte Arduino\\ automate};
\node[bloc] (com) at (4.7,0)  {\textbf{COMMUNIQUER}\\[3pt] voyants, écran\\ relais, transistors\\ réseau};
...
%% ---- entrée et sortie ------------------------------------------------
\draw[fleche] (-2.55,0) -- (acq.west);
\node[bord, align=right, anchor=south east] at (-1.45,0.12) {Grandeurs physiques, consignes};
\draw[fleche] (com.east) -- (7.15,0);
\node[bord, align=left, anchor=south west] at (6.1,0.12) {Informations (ordres, messages)};
\end{tikzpicture}
\end{document}
