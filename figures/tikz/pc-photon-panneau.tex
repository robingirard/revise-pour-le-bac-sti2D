...
\node[titre] at (1.05,2.92) {Absorption d'un photon};

%% niveaux d'énergie
\draw[niv] (0.35,0.55) -- (1.85,0.55);
\draw[niv] (0.35,2.15) -- (1.85,2.15);
\node[font=\small, anchor=west] at (1.92,0.55) {$E_1$};
\node[font=\small, anchor=west] at (1.92,2.15) {$E_2$};

%% photon incident : onde entretenue puis pointe de flèche
\draw[solideD, line width=1pt] plot[domain=-0.50:0.65, samples=70, smooth]
     (\x,{1.35+0.13*sin(deg((\x+0.50)*2*pi/0.2875))});
\draw[-{Stealth[length=5pt]}, solideD, line width=1pt] (0.65,1.35) -- (0.85,1.35);
\node[font=\scriptsize, text=solideD, anchor=south] at (0.05,1.56) {photon\rappel{ $E = h\nu$}};

%% saut de l'électron (disque noir : l'électron)
\draw[-{Stealth[length=6pt]}, line width=1.2pt, solideA] (1.15,0.62) -- (1.15,2.08);
\draw[fill=black!30, draw=black!60, line width=0.4pt] (1.15,0.55) circle (0.085);
\fill[black] (1.15,2.15) circle (0.085);
\node[font=\scriptsize\bfseries, text=solideA, anchor=west] at (1.26,1.35) {absorption};

\node[note, anchor=north] at (1.05,0.22)
