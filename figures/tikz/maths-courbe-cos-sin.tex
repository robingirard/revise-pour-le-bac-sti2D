...
\node[font=\small, text=solideA, anchor=south west, inner sep=2pt] at (0.2,1.12) {$\mathcal{C}_{\cos}$};
\node[font=\small, text=solideB, anchor=south east, inner sep=2pt] at (3.2,0.28) {$\mathcal{C}_{\sin}$};
...
%% ---------------- légende ------------------------------------------------
\begin{scope}[shift={(1.05,1.02)}]
  \draw[fill=white, draw=black!35, line width=0.5pt, rounded corners=2pt]
       (0,0) rectangle (3.35,0.86);
  \draw[ccos] (0.14,0.6) -- (0.7,0.6);
  \node[font=\small, anchor=west, inner sep=2pt] at (0.78,0.6) {$\mathcal{C}_{\cos}$ : paire};
  \draw[csin] (0.14,0.25) -- (0.7,0.25);
  \node[font=\small, anchor=west, inner sep=2pt] at (0.78,0.25) {$\mathcal{C}_{\sin}$ : impaire};
\end{scope}
\end{tikzpicture}
\end{document}
