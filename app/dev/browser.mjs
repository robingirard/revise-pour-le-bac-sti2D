...
window.__solve = function () {
  const plain = (s) => String(s).replace(/\\*\\*|\\*|\\$/g, '').replace(/\\{\\{fig:[^}]+\\}\\}/g, '').replace(/\\{\\{emoji:[^}]+\\}\\}/g, '').replace(/\\s+/g, ' ').trim();
  ...
  // répond juste à un exercice de type « kind » (payload p) monté dans « root »
  const solveKind = (kind, p, root) => {
    if (kind === 'flashcard') { root.querySelector('.btn-primary').click(); root.querySelector('.grade-good').click(); return 'flashcard'; }
    if (kind === 'mcq') { for (const i of p.answer) findBtn(root.querySelectorAll('.choice'), p.choices[i]).click(); root.querySelector('.btn-verify').click(); return 'mcq'; }
    if (kind === 'grid') { for (const id of p.answer) root.querySelector('input[data-cell="' + id + '"], input[aria-label="' + ((p.labels && p.labels[id]) || id) + '"]').click(); root.querySelector('.btn-verify').click(); return 'grid'; }
    if (kind === 'order') { for (const s of p.steps) findBtn(root.querySelectorAll('.order-pool .choice'), s).click(); root.querySelector('.btn-verify').click(); return 'order'; }
    if (kind === 'input') { const inp = root.querySelector('.text-input'); inp.value = p.answer; inp.dispatchEvent(new Event('input')); root.querySelector('.btn-verify').click(); return 'input'; }
    if (kind === 'match') {
  ...
  if (type === 'guided') {
    const item = window.CONTENT.items[box.dataset.item];
    const stepBox = box.querySelector('.guided-step');
    if (!item || !stepBox || stepBox.dataset.step == null) return 'guided-no-step';
    if (box.querySelector('.btn-next-step:not(:disabled)')) return 'guided-step-done';
    const step = item.payload.steps[Number(stepBox.dataset.step)];
    return 'guided:' + solveKind(step.kind || 'mcq', step, stepBox);
  }
  ...
  return solveKind(type, item.payload, box);
};`;
